...
(async()=>{
  ...
  try{
    ...
    const openBenchResponse=await json(base,'/api/projects/p_lora/comments',{method:'POST',cookie:mike,body:{body:'Open Bench · Looking for Tester\n\nI can run a two-night outdoor condensation test.'}});check('Open Bench response uses the project discussion',openBenchResponse.res.status===201&&openBenchResponse.data.comment?.body.includes('Looking for Tester'));
    r=await json(base,'/api/projects/p_lora');check('Open Bench response remains attached to the public project',r.res.ok&&r.data.comments.some(c=>c.id===openBenchResponse.data.comment?.id));
