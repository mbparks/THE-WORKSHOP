...
(async()=>{
  ...
  try{
    ...
    await setHash(cdp,'#/trails',`Boolean(document.querySelector('.quest-trail-library-section'))`);
    check('Quest Trail hub renders ordered paths without public progress totals',await evaluate(cdp,`(()=>{const t=document.querySelector('.quest-trails-view')?.textContent||'';return document.querySelectorAll('.quest-trail-card').length>=3&&t.includes('Quest Trails')&&t.includes('ORDERED, NOT RANKED')&&t.includes('PRIVATE PROGRESS')&&t.includes('NO COMPLETION COUNTS')&&!/\b\d+\s+completions?\b/i.test(t)})()`));
    await setHash(cdp,'#/trail/trail_walk_into_making',`Boolean(document.querySelector('.quest-trail-detail-view'))`);
    check('Quest Trail detail renders the sequence and private progress boundary',await evaluate(cdp,`(()=>{const t=document.querySelector('.quest-trail-detail-view')?.textContent||'';return document.querySelectorAll('.quest-trail-step').length===3&&t.includes('Walk the trail')&&t.includes('PRIVATE TO YOU')&&t.includes('Approximate only · never an address')&&t.includes('NO SCORE')})()`));
    await setHash(cdp,'#/trails/mine',`Boolean(document.querySelector('.quest-trail-library-section'))`);
    await evaluate(cdp,`document.querySelector('[data-action="new-trail"]')?.click()`);
    await waitForCondition(cdp,`Boolean(document.querySelector('#quest-trail-form'))`,'Quest Trail editor');
    check('Quest Trail editor enforces ordered steps and approximate place privacy',await evaluate(cdp,`(()=>{const f=document.querySelector('#quest-trail-form'),t=f?.textContent||'';return t.includes('2–12 STEPS')&&t.includes('Why this step is here')&&t.includes('Place hints stay approximate')&&!f.querySelector('[name="address"]')&&!f.querySelector('[name="latitude"]')&&!f.querySelector('[name="longitude"]')})()`));
    await evaluate(cdp,`document.querySelector('[data-action="close-overlay"]')?.click()`);
