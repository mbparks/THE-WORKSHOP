...
const checks=[
  ...
  ,['External commerce channels include Etsy, RedBubble, and Amazon Books',html.includes('https://www.etsy.com/shop/GreenShoeGarage')&&html.includes('redbubble.com/people/GreenShoeGarage/shop')&&html.includes('a.co/d/09kRqjO5')&&html.includes('ETSY SHOP')&&html.includes('REDBUBBLE')&&html.includes('AMAZON BOOKS')&&html.includes('target="_blank"')]
  ...
  ,['Reddit community link exists site-wide',html.includes('https://www.reddit.com/r/greenshoegarage/')&&app.includes('https://www.reddit.com/r/greenshoegarage/')&&html.includes('>REDDIT <')&&app.includes('>REDDIT <')]
  ,['Repair Clinic link follows Parts Library in navigation',html.includes('https://greenshoegarage.com/repairclinic/')&&app.includes('https://greenshoegarage.com/repairclinic/')&&html.indexOf('PARTS LIBRARY')<html.indexOf('REPAIR CLINIC')&&app.indexOf('PARTS LIBRARY')<app.indexOf('REPAIR CLINIC')]
  ,['Sideband Radio link follows Repair Clinic in navigation',html.includes('https://greenshoegarage.com/radio')&&app.includes('https://greenshoegarage.com/radio')&&html.indexOf('REPAIR CLINIC')<html.indexOf('Sideband Radio')&&app.indexOf('REPAIR CLINIC')<app.indexOf('Sideband Radio')]
  ...
  ['About explains commerce as income plus discovery',app.includes('SUPPORT + DISCOVER')&&app.includes('modest revenue')&&app.includes('way for people beyond THE WORKSHOP to discover')&&app.includes('OPEN ETSY ↗')&&app.includes('OPEN REDBUBBLE ↗')&&app.includes('FIND BOOKS ON AMAZON ↗')]
  ...
  ,['Unfinished work is integrated into Project and Make Together surfaces',server.includes('unfinished')&&server.includes('commons')&&server.includes('comments')&&app.includes('id="project-unfinished"')&&app.includes('unfinished-public-section')]
  ...
  ,['Commons has additive exchange and private-response persistence',server.includes('CREATE TABLE IF NOT EXISTS commons_entries')&&server.includes('CREATE TABLE IF NOT EXISTS commons_responses')&&server.includes('idx_commons_responses_entry')]
  ,['Commons uses four bounded exchange types and scopes',server.includes("new Set(['Have','Need','Teach','Help'])")&&server.includes("new Set(['Workshop','Local','Remote','My Crew'])")&&app.includes('I CAN TEACH')&&app.includes('I CAN HELP')]
  ,['Commons visibility and project/Crew context are server enforced',server.includes('commonsVisible')&&server.includes('canViewLinkedProject')&&server.includes('canUseCommonsCrew')&&server.includes('Local exchanges need an approximate city or region')]
  ,['Commons responses are private and owner managed',server.includes('commonsResponseMatch')&&server.includes('Only the Commons author can manage responses')&&app.includes('PRIVATE THREAD')&&app.includes('RESPOND PRIVATELY')]
  ,['Commons is integrated into existing Workshop surfaces',server.includes('commons')&&server.includes('comments')&&app.includes("route==='commons'")&&app.includes('id="project-commons"')&&app.includes('commons-public-section')&&app.includes('crew-commons')]
  ,['Commons stays chronological and unranked',server.includes("orderedBy:'most recently updated'")&&server.includes('countsPublic:false')&&app.includes('CHRONOLOGICAL · NO COUNTS')&&!app.includes('commonsResponseCount')]
  ,['v10.2 Commons capability remains published',server.includes('commons:true')&&server.includes('handoffFieldCards:true')]
  ,['Handoff Field Cards have additive persistence and indexes',server.includes('CREATE TABLE IF NOT EXISTS handoff_cards')&&server.includes('CREATE TABLE IF NOT EXISTS handoff_feedback')&&server.includes('idx_handoff_cards_project')&&server.includes('idx_handoff_feedback_card')]
  ,['Handoff Field Cards expose durable API routes',server.includes("pathname==='/api/handoff'")&&server.includes('/feedback')&&server.includes('handoffVisibility')&&server.includes('HANDOFF_STATUSES')]
  ,['Handoff Field Cards enforce linked Project and Crew access',server.includes('canViewProject(project,u)')&&server.includes('crewRole(crewId,u.id)')&&server.includes('handoffVisible')]
