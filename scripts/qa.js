...
const checks=[
  ...
  ,['Browser QA exercises route persistence and real editors',browserQa.includes('Atmosphere recomposes between major modules')&&browserQa.includes('Shared media picker is available in the project editor')&&browserQa.includes('Quest Trail hub renders ordered paths')]
  ...
  ,['Local Quests have additive persistence and bounded vocabularies',server.includes('CREATE TABLE IF NOT EXISTS local_quests')&&server.includes('CREATE TABLE IF NOT EXISTS local_quest_attempts')&&server.includes('idx_local_quest_attempts_user')&&server.includes("const LOCAL_QUEST_TYPES=['Notice','Meet','Repair','Make','Explore','Document']")&&server.includes("new Set(['Started','Completed','Abandoned'])")]
  ,['Local Quest authoring and attempt routes are durable',server.includes("pathname==='/api/quests'")&&server.includes('/start$/')&&server.includes('questAttemptMatch')&&server.includes('Only the quest maker or a Workshop editor')&&server.includes('Only the maker of this field note')]
  ,['Local Quest visibility and context links are server enforced',server.includes('localQuestVisible')&&server.includes('localQuestLinkAccess')&&server.includes('localQuestBoundVisibility')&&server.includes('place_hint')&&app.includes('Approximate only · never an address')]
  ,['Local Quest attempts are private until a completed note is intentionally shared',server.includes("visibility=status==='Completed'?localQuestBoundVisibility")&&server.includes('quest.user_id!==u.id')&&app.includes('Draft notes stay private')&&app.includes('Public or Member visibility applies only after completion')]
  ,['Local Quests avoid precise location and popularity mechanics',server.includes('place_hint')&&app.includes('Do not include an address, coordinates')&&server.includes('countsPublic:false')&&!app.includes('questCompletionCount')&&!app.includes('questFollowerCount')]
  ,['Local Quests integrate into Workshop discovery and context surfaces',app.includes("route==='quest'")&&app.includes("route==='quests'")&&app.includes('home-local-quests')&&app.includes('quest-public-section')&&app.includes('project-quests-section')&&app.includes('crew-quests-section')&&app.includes('SEARCH_KINDS')]
  ,['Local Quests support saves search export and reset cleanup',server.includes("item_type='quest'")&&server.includes("if(want('quests'))")&&server.includes('localQuestAttempts')&&server.includes('DELETE FROM local_quest_attempts')&&app.includes('save-quest')]
  ,['v10.6 capability flag and version markers are published',server.includes('questTrails:true')&&server.includes("const APP_VERSION = '10.6.0'")&&pkg.version==='10.6.0'&&read('public/index.html').includes('v10.6.0')&&read('public/manifest.webmanifest').includes('10.6.0')&&read('public/sw.js').includes('workshop-v10.6.0')]
  ,['Quest Trails have additive persistence and bounded ordered steps',server.includes('CREATE TABLE IF NOT EXISTS local_quest_trails')&&server.includes('CREATE TABLE IF NOT EXISTS local_quest_trail_steps')&&server.includes('idx_local_quest_trail_steps_trail')&&server.includes('LOCAL_TRAIL_MAX_STEPS')&&server.includes('A Quest Trail cannot repeat the same Local Quest')]
  ,['Quest Trail access stays bounded and private progress is unranked',server.includes('localTrailBoundVisibility')&&server.includes('localTrailVisible')&&server.includes('myProgress')&&server.includes('countsPublic:false')&&app.includes('NO COMPLETION COUNTS')&&app.includes('NO PRECISE LOCATIONS')&&!app.includes('trailCompletionCount')]
  ,['Quest Trails expose authoring, save, edit, and delete routes',server.includes("pathname==='/api/quest-trails'")&&server.includes('questTrailSave')&&server.includes('questTrailMatch')&&app.includes('questTrailForm')&&app.includes('save-trail')]
  ,['Quest Trails integrate into discovery and context surfaces',app.includes("route==='trail'")&&app.includes("route==='trails'")&&app.includes('home-quest-trails')&&app.includes('quest-trail-public-section')&&app.includes('project-quest-trails-section')&&app.includes('crew-quest-trails-section')&&app.includes("['trails','Quest Trails']")]
  ,['Quest Trails support saves search export and reset cleanup',server.includes("item_type='quest-trail'")&&server.includes("if(want('trails'))")&&server.includes('localQuestTrailSteps')&&server.includes('DELETE FROM local_quest_trails')&&app.includes('save-trail')]
