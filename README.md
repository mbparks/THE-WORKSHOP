...
## v5.8.2 — Transactional Notifications
...
Run `npm run qa` before deployment. The v5.8.2 suite currently contains **54 static release checks**, with separate runtime smoke tests used during development for privacy, migration, and Crew lifecycle verification.
...
git branch -M main
git add .
git commit -m "Initial THE WORKSHOP v5.8.2 release"
...
```bash
git tag -a v5.8.2 -m "THE WORKSHOP v5.8.2"
